...
use crate::{
    estree,
    expression::{
        AssignmentExpression, Expression, ExpressionStatement, Initializer, LeftHandSideExpression,
        PropertyName,
    },
...
fn ClassDeclaration2(
    ...
    let body = ClassBody(bracket.0)?;
    let _bracket = is_token!(body.0, Token::RCurly)?;
...
fn FormalParameters5(
    ...
    let comma = is_token!(list.0, Token::Comma)?;
    let rest = BindingRestElement(comma.0)?;
    list.1.push(rest.1);
...
mod test {
    use super::*;
    use crate::{
        estree::{Identifier, VariableDeclaration, *},
