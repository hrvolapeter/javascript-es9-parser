...
#[inline]
fn ExponentPart(input: CompleteStr) -> IResult<CompleteStr, i128> {
    let indicator = one_of!(input, "eE")?;
    SignedInteger(indicator.0)
}

#[inline]
fn SignedInteger(input: CompleteStr) -> IResult<CompleteStr, i128> {
    ...
    let mut integer: (_, i128) = (integer.0, (*integer.1).parse().unwrap());
    if let Ok(sign) = sign {
        if sign.1 == '-' {
            integer.1 *= -1;
        }
...
fn StringLiteralDouble(input: CompleteStr) -> ParseResult {
    ...
    let unescaped = unescape(&strr[..]).unwrap_or(String::new());

    Ok((input.0, Token::StringLiteral(unescaped)))
}
...
fn StringLiteralSingle(input: CompleteStr) -> ParseResult {
    ...
    let unescaped = unescape(&string[..]).unwrap_or(String::new());
